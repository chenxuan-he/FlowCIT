...
def test(args, epoch, R_net, testLoader, optimizer, device):
    ...
    data_list = []  # Store all data batches
    latent_list = []
    target_list = []  # Store all target batches
    ...
    with torch.no_grad():
        for data, target in testLoader:
            ...
            # Store batch data/target (move to CPU first)
    ...
    target_np = np.concatenate(target_list, axis=0) #.squeeze()  # Remove extra dimensions
    
    # Create DataFrame and save to CSV
    df_x = pd.DataFrame(
        data=data_np,
        columns=[f'x_{i}' for i in range(data_np.shape[1])]
    )
    df_x.to_csv(f'results_ddr/test_results_epoch_x.csv', index=False)

    df_y = pd.DataFrame(
        data=target_np,
        columns=[f'y_{i}' for i in range(target_np.shape[1])]
    )
    df_y.to_csv(f'results_ddr/test_results_epoch_y.csv', index=False)
